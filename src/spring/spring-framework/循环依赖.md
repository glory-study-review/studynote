...
2. `earlySingletonObjects`：提前曝光的单例对象的cache，存放原始的 bean 对象（尚未填充属性），如果存在循环依赖 会通过第三级缓存生成早期应用存放在此。
...
这个创建前校验，往`singletonsCurrentlyInCreation`中添加了当前beanName，此处也是循环
...
- 初始化
  ...
  属性填充成功后，会进行初始化，会进行bean的后置处理器处理，如代理对象的创建
...
https://www.yisu.com/jc/724480.html
